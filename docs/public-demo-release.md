...
The selected settings are `MAX_EPOCH_AGE=2592000` (30 days) and
`MIN_EPOCH_INTERVAL=60`, confirmed on 25 September 2026. Set these **before deployment**.
...
On 25 September 2026, the user made the repository public and configured
**Settings → Pages → Build and deployment → Source: GitHub Actions**.
The GitHub API confirmed public visibility and Pages availability.
...
Review the complete tested `OpenSolvency` project and its release records before
integrating it into `main`. The workflow validates each push/PR. Only a push to
