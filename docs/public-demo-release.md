...
The proposed settings remain `MAX_EPOCH_AGE=604800` (seven days) and
`MIN_EPOCH_INTERVAL=60`. Publish at least every five days during active grading.
If the team chooses 30 days instead, set `MAX_EPOCH_AGE=2592000` **before deployment**.
...
The repository is currently private. GitHub Pages for a private repository requires
a supported paid plan (for example GitHub Pro for a personal repository). Confirm
Pages availability in the owner account before release; do not change repository
visibility merely to enable hosting. A separate hosting decision is needed if the
current plan does not support it.

The repository owner must select **Settings → Pages → Build and deployment →
Source: GitHub Actions**. Configure the `github-pages` environment to allow `main`.
...
Review this feature branch against `OpenSolvency`, then integrate the complete
tested project into `main`. The workflow validates each push/PR. Only a push to
