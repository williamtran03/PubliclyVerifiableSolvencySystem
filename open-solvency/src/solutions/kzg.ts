...
export const kzg: Solution = {
  ...
  publication: ["Prepare the SRS once: make kzg-setup extracts it from the pinned Perpetual Powers of Tau file and checks it. Keep srs.json; replacing it invalidates every earlier proof.", "Before each epoch: re-prove control of every reserve wallet, then have the auditor call sampleReserves.", "Deploy the registry first, then record its address, chain ID and next epoch ID in a snapshot.json. The proof transcript binds to that address.", "Build the epoch: make kzg-epoch SNAPSHOT=<snapshot.json> OUT=<directory>. This writes four files: epoch.json, range-proof.json, inclusion.json and attack.json. Omitting both variables overwrites the committed fixtures.", "Review the artifacts before signing.", "Call submitEpoch with the company key and deliver each customer opening from inclusion.json privately.", "For a local end-to-end run of all of the above: make kzg-demo", "On Sepolia, npm run sepolia -- epoch snarkless runs all of the above with the keys in .env"],
