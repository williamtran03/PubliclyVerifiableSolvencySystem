...
test("company audit recomputes the public ledger, then rejects changed balances", () => {
  ...
  assert.throws(() => inspectArtifact("published-ledger", snapshot, JSON.stringify(artifact)), /does not match/);
});
...
test("company ZK artifact must match root, context and every floor", () => {
  ...
  assert.match(inspectArtifact("zk-circuit", snapshot, JSON.stringify({ rootHash: "7", context: "8", floors: ["90"] })), /match/);
  assert.throws(() => inspectArtifact("zk-circuit", snapshot, JSON.stringify({ rootHash: "7", context: "8", floors: ["91"] })), /does not match/);
});
