...
export const zk: Solution = {
  ...
  publication: ["Before each epoch: re-prove control of every reserve wallet, then have the auditor call sampleReserves.", "Fetch the snapshot and reserve context: make zk-snapshot REGISTRY=0x…", "Build the witness and customer bundles: make zk-fixtures", "Generate and verify the proof: make zk-prove", "Publish with the company key: make zk-demo (local demo)", "On Sepolia, npm run sepolia -- epoch zk-circuit runs all of the above with the keys in .env"],
