...
export async function localSetup(rpc='http://127.0.0.1:8545') {
 ...
 async function prepare(id:Hex,raw=50000000000000000n){return buildSnapshot([{customerId:'demo-alice',dateOfBirth:'2000-01-01',holdings:[{token:NATIVE,rawAmount:raw}]},{customerId:'demo-bob',dateOfBirth:'1990-02-02',holdings:[{token:NATIVE,rawAmount:25000000000000000n}]}],new MockOracle([rate]),id,block.timestamp,3600n,16,2,4);}
 async function submit(snapshot:Awaited<ReturnType<typeof prepare>>) {const l=snapshot.ledger;await send(company,'addLiability',[{id:l.snapshotId,rootHash:l.rootHash,totalLiabilitiesUsd:l.rootSum,rateManifestHash:snapshot.rateManifestHash,snapshotTime:block.timestamp,snapshotBlock:block.number},l.pairs.map(p=>p.identity),l.pairs.map(p=>p.amount),snapshot.manifest]);await send(auditor,'verifyAddLiability',[l.snapshotId,true]);}
