...
export async function localSetup(rpc='http://127.0.0.1:8545') {
 ...
 async function prepare(id:Hex,raw=50000000000000000n){const pinned=await client.readContract({address:registry,abi:minimumAbi as Abi,functionName:'getRates',args:[id]});return buildSnapshot([{customerId:'demo-alice',dateOfBirth:'2000-01-01',holdings:[{token:NATIVE,rawAmount:raw}]},{customerId:'demo-bob',dateOfBirth:'1990-02-02',holdings:[{token:NATIVE,rawAmount:25000000000000000n}]}],new MockOracle(pinned as Rate[]),id,block.timestamp,3600n,16,2,4);}
 ...
 const snapshotId=`0x${randomBytes(32).toString('hex')}` as Hex;await send(company,'pinRates',[snapshotId,block.timestamp,[rate]]);const snapshot=await prepare(snapshotId);await submit(snapshot);
