...
contract MerkleSumRegistryTest is Test {
    ...
    function test_ComputesAndStoresRoot() public {
        ...
        assertEq(storedSum, sum);
        assertEq(registry.epochCount(), 1);
    }

    function test_RejectsLegacyRoute() public {
        vm.expectRevert("use submitLedger");
        registry.submitEpoch(123, 1);
    }
    ...
    function test_RejectsInsolvencyOverflowAndInvalidLength() public {
        ...
        vm.expectRevert();
        registry.computeRoot(ids, amounts);
        vm.expectRevert("invalid length");
        registry.computeRoot(ids, new uint256[](0));
        vm.expectRevert("invalid length");
    ...
    function test_RejectsDuplicateReserves() public {
        ...
        reserves[0] = address(1);
        reserves[1] = address(1);
        vm.expectRevert("duplicate reserve");
        new MerkleSumRegistry(reserves);
    }
}
