...
test("a range proof does not carry to another epoch or deployment", () => {
  const proof = proveRange(srs, balancePoly, balances, CTX, BITS);
  assert.ok(verifyRange(srs, balanceCommitment, balances.length, proof, CTX));
  assert.equal(verifyRange(srs, balanceCommitment, balances.length, proof, CTX + 1n), false);
});
...
test("a balance that does not fit in the bit width is rejected up front", () => {
  ...
  assert.throws(() => proveRange(srs, poly, tooBig, CTX, BITS), /does not fit in 8 bits/);
});
...
test("bits that don't actually reconstruct the balance can't even produce a proof", () => {
  ...
  assert.throws(() => proveRange(srs, mismatchedPoly, balances, CTX, BITS), /not divisible/);
});

test("bit commitments are blinded, so the same balances never commit twice the same way", () => {
  const first = proveRange(srs, balancePoly, balances, CTX, BITS);
  const second = proveRange(srs, balancePoly, balances, CTX, BITS);
  ...
  assert.ok(verifyRange(srs, balanceCommitment, balances.length, first, CTX));
  assert.ok(verifyRange(srs, balanceCommitment, balances.length, second, CTX));
});
...
test("tampering with one opened value after the fact breaks verification", () => {
  ...
  assert.equal(verifyRange(srs, balanceCommitment, balances.length, tampered, CTX), false);
});
