...
import { buildSplitLiabilities, verifyCustomer } from "../prover/splitLiabilities.ts";
...
const customers = JSON.parse(readFileSync("arms/published-ledger/fixtures/customers.example.json", "utf8")).map((c: any) => ({ ...c, amounts: c.amounts.map(BigInt) }));
